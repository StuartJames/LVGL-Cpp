...
enum EG_DisplayRotation_t : uint8_t{
    EG_DISP_ROT_NONE = 0,
    EG_DISP_ROT_90,
    EG_DISP_ROT_180,
    EG_DISP_ROT_270
...
public:
                  ...
                  ~EGDisplayDriver(void);
  virtual void    InitialiseContext(EGDeviceContext *pDC);
  virtual void    DeinitialiseContext(EGDeviceContext *pDC);
  ...
  void            (*SetPixelCB)(EGDisplayDriver *pDriver, uint8_t *pBuffer, int32_t Width, int32_t X, int32_t Y, 
...
//  void            (*InitialiseContext)(EGDisplayDriver *pDriver, EGDeviceContext *pDC);  // now performed in constructor...
//  void            (*DinitialiseContext)(EGDisplayDriver *pDriver, EGDeviceContext *pDC);  // and in destructor.

  int32_t             m_HorizontalRes;          // Horizontal resolution.
  int32_t             m_VerticalRes;            // Vertical resolution.
  int32_t             m_PhysicalHorizontalRes;  // Horizontal resolution of the full / physical display. Set to -1 for fullscreen mode.
  int32_t             m_PhysicalVerticalRes;    // Vertical resolution of the full / physical display. Set to -1 for fullscreen mode.
  int32_t             m_OffsetX;                // Horizontal offset from the full / physical display. Set to 0 for fullscreen mode.
  int32_t             m_OffsetY;                // Vertical offset from the full / physical display. Set to 0 for fullscreen mode.
  EG_DisplayDrawBuffer_t *m_pDrawBuffers;       // Pointer to a buffer.
