...
#if EG_USE_LABEL == 0
#error "EGDropdown: EGLabel are required. Enable it in EG_Config.h (EG_USE_LABEL 1)"
#endif
...
private:
  ...
  void              DrawBox(EGDeviceContext *pDC, uint16_t Id, EGState_t State);
  void              DrawBoxLabel(EGDeviceContext *pDC, uint16_t ID, EGState_t State);
  uint16_t          GetIndexOnPoint(int32_t Y);
