...
enum EG_StyleStateCmp_e : uint8_t {
...
typedef struct Transition_t{
  Transition_t(void) : pObj(nullptr), Property(EG_STYLE_PROP_INV), SelectFlags(0){ StartValue.Number = 0; EndValue.Number = 0;}
	EGObject            *pObj;
...
typedef struct EG_TransitionDescriptor_t{
  EG_TransitionDescriptor_t(void) : Time(0), Delay(0), SelectFlags(0), Property(EG_STYLE_PROP_INV), PathCB(nullptr), m_pParam(nullptr){}
  uint16_t              Time;
  uint16_t              Delay;
  EG_StyleFlags_t       SelectFlags;
  EGStyleProperty_e     Property;
