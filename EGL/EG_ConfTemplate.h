...
#define EG_COLOR_16_SWAP 1

// Enable features to draw on transparent background.
// It's required if opa, and transform_* style properties are used.
// Can be also used if the UI is above another layer, e.g. an OSD menu or video player.
#define EG_COLOR_SCREEN_TRANSP 1

// Adjust color mix functions rounding. GPUs might calculate color mix (blending) differently.
// 0: round down, 64: round up from x.75, 128: round up from half, 192: round up from x.25, 254: round up
#define EG_COLOR_MIX_ROUND_OFS 128
...
#if EG_MEM_CUSTOM == 0  // not set yet
#define EG_MEM_CUSTOM 0  // 1: use custom malloc/free, 0: use the built-in `EG_AllocMem()` and `EG_FreeMem()`
#if EG_MEM_CUSTOM == 0
    ...
    // Instead of an address give a memory allocator that will be called to get a memory pool for EGL. E.g. my_malloc
    #if EG_MEM_ADR == 0
...
#if EG_TICK_CUSTOM == 0 // not set yet
...
#define EG_TICK_CUSTOM 0
    ...
    // If using EGL as ESP32 component
...
#define EG_USE_GPU_STM32_DMA2D 0
    ...
    #define EG_GPU_DMA2D_CMSIS_INCLUDE "stm32f743.h"
#endif
...
#define EG_USE_GPU_NXP_PXP 0
    ...
    /*1: Add default bare metal and FreeRTOS interrupt handling routines for PXP (EG_PXP_OSA.cpp)
    *   and call EGPXPInitialise() automatically during EG_Init(). Note that symbol SDK_OS_FREE_RTOS
    *   has to be defined in order to use FreeRTOS OSA, otherwise bare-metal implementation is selected.
    *0: EGPXPInitialise() has to be called manually before EG_Init()
    */
    #define EG_USE_GPU_NXP_PXP_AUTO_INIT 1
#endif
...
#if EG_USE_GPU_SDL
    #define EG_GPU_SDL_INCLUDE_PATH <SDL.h>
...
#if EG_SPRINTF_CUSTOM == 0
#define EG_SPRINTF_CUSTOM 0 // Change the built in (v)snprintf functions
...
// Used if EGL is bound to higher level language and the memory is managed by that language
...
#define EG_FONT_MONTSERRAT_20 1
#define EG_FONT_MONTSERRAT_22 1
#define EG_FONT_MONTSERRAT_24 1
...
#if EG_USE_CALENDAR
    #define EG_CALENDAR_WEEK_STARTS_MONDAY 1
...
#define EG_USE_FS_STDIO 0
    ...
    #define EG_FS_STDIO_CACHE_SIZE 0    // >0 to cache this number of bytes in EG_FileSystem.Read()
#endif
...
#define EG_USE_FS_POSIX 0
    ...
    #define EG_FS_POSIX_CACHE_SIZE 0    // >0 to cache this number of bytes in EG_FileSystem()
#endif
...
#define EG_USE_FS_WIN32 0
    ...
    #define EG_FS_WIN32_CACHE_SIZE 0    // >0 to cache this number of bytes in EG_FileSystem()
#endif
...
#define EG_USE_FS_FATFS 0
    ...
    #define EG_FS_FATFS_CACHE_SIZE 0    // >0 to cache this number of bytes in EG_FileSystem()
#endif
...
#define EG_USE_FS_LITTLEFS 0
    ...
    #define EG_FS_LITTLEFS_CACHE_SIZE 0    // >0 to cache this number of bytes in EG_FileSystem()
#endif
