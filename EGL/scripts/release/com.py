...
def get_lvgl_version():
    ...
    for i in f.read().splitlines():
        r = re.search(r'^#define EG_VERSION_MAJOR ', i)
        if r:
            m = lastNum.search(i)
            if m: ver[0] = m.group(1)

        r = re.search(r'^#define EG_VERSION_MINOR ', i)
        if r:
            m = lastNum.search(i)
            if m: ver[1] = m.group(1)

        r = re.search(r'^#define EG_VERSION_PATCH ', i)
        if r:
            m = lastNum.search(i)
...
def update_version(ver):
  ...
  if path.exists("EGL.h"):
      define_set("./EGL.h", "EG_VERSION_MAJOR", str(ver[0]))
      define_set("./EGL.h", "EG_VERSION_MINOR", str(ver[1]))
      define_set("./EGL.h", "EG_VERSION_PATCH", str(ver[2]))
      define_set("./EGL.h", "EG_VERSION_INFO", "\"" + ver[3] + "\"")
