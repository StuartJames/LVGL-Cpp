...
uint32_t ch_enc;
	...
	txt_length = EG_TextEncodedGetLength(txt);
	i = 0;
	j = 0;
	while(i < txt_length) {
		ch_enc = EG_TextDecodeNext(txt, &j);
		current_ap_idx = lv_ap_get_char_index(ch_enc);
		if(current_ap_idx != EG_UNDEF_ARABIC_PERSIAN_CHARS)	ch_enc = ap_chars_map[current_ap_idx].char_end_form;
		if(ch_enc < 0x80)	chars_cnt++;
		else if(ch_enc < 0x0800) chars_cnt += 2;
		else if(ch_enc < 0x010000) chars_cnt += 3;
		else chars_cnt += 4;
		i++;
	}
	return chars_cnt + 1;
...
char *txt_out_temp;
	...
	txt_length = EG_TextEncodedGetLength(txt);
	ch_enc = (uint32_t *)EG_AllocMem(sizeof(uint32_t) * (txt_length + 1));
	ch_fin = (uint32_t *)EG_AllocMem(sizeof(uint32_t) * (txt_length + 1));
	i = 0;
	j = 0;
	while(j < txt_length)	ch_enc[j++] = EG_TextDecodeNext(txt, &i);
	ch_enc[j] = 0;
	i = 0;
	j = 0;
	idx_previous = EG_UNDEF_ARABIC_PERSIAN_CHARS;
	while(i < txt_length) {
		...
		if(index_current == EG_UNDEF_ARABIC_PERSIAN_CHARS) {
			ch_fin[j] = ch_enc[i];
			j++;
			i++;
			idx_previous = EG_UNDEF_ARABIC_PERSIAN_CHARS;
		...
		uint8_t conjunction_to_previuse = (i == 0 || idx_previous == EG_UNDEF_ARABIC_PERSIAN_CHARS) ?	0 :
			        ap_chars_map[idx_previous].ap_chars_conjunction.conj_to_next;
		uint8_t conjunction_to_next = ((i == txt_length - 1) || idx_next == EG_UNDEF_ARABIC_PERSIAN_CHARS) ? 0 :
...
static uint32_t lv_ap_get_char_index(uint16_t c)
{
	for(uint8_t i = 0; ap_chars_map[i].char_end_form; i++) {
		if(c == (ap_chars_map[i].char_offset + LV_AP_ALPHABET_BASE_CODE))	return i;
	...
	return EG_UNDEF_ARABIC_PERSIAN_CHARS;
}
...
uint32_t ch_code = 0;
	...
	if(ch_next == EG_UNDEF_ARABIC_PERSIAN_CHARS) {
		return 0;
	}
