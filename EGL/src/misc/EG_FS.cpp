...
void EGFile::operator=(const EGFile &rval)
{
...
void EGFile::Register(EGFileDriver *pDriver)
{
...
EGFileDriver* EGFile::GetDriver(char DriverID)
{
...
bool EGFile::IsReady(char DriverID)
{
...
EG_FSResult_e EGFile::Open(const char *pPath, EG_FS_Mode_e Mode)
{
	if(pPath == nullptr) {
...
EG_FSResult_e EGFile::Close(void)
{
...
EG_FSResult_e EGFile::ReadCached(char *pReadBuffer, uint32_t Count, uint32_t *pReadCount)
{
...
EG_FSResult_e EGFile::Read(void *pBuffer, uint32_t Count, uint32_t *pReadCount)
{
...
EG_FSResult_e EGFile::Write(const void *pBuffer, uint32_t Count, uint32_t *pWriteCount)
{
...
EG_FSResult_e EGFile::Seek(uint32_t Pos, EG_FS_Seek_e Mode)
{
...
EG_FSResult_e EGFile::Tell(uint32_t *pPos)
{
EG_FSResult_e Result;
...
EG_FSResult_e EGFile::DirOpen(const char *pPath)
{
	...
	if(Driver->DirOpenCB == nullptr) {
		return EG_FS_RES_NOT_IMP;
	}
	const char *pRealPath = IsolatePath(pPath);
	void *pDir = Driver->DirOpenCB(Driver, pRealPath);
	if(pDir == nullptr || pDir == (void *)(-1)) {
		return EG_FS_RES_UNKNOWN;
	}
	m_pDriver = Driver;
	m_pDir = pDir;
	return EG_FS_RES_OK;
...
EG_FSResult_e EGFile::DirRead(char *pFileNames)
{
...
EG_FSResult_e EGFile::DirClose(void)
{
	if(m_pDriver == nullptr || m_pDir == nullptr) {
		return EG_FS_RES_INV_PARAM;
	}
...
char* EGFile::GetDrives(char *pBuffer)
{
EGFileDriver *pDriver;
uint8_t i = 0;
...
const char* EGFile::GetExt(const char *pFileName)
{
...
const char* EGFile::GetLast(const char *pPath)
{
size_t i = 0;
...
const char* EGFile::IsolatePath(const char *pPath)
{
