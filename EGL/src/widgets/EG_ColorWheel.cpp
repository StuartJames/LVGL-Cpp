...
#include "widgets/EG_ColorWheel.h"
...
void EGColorWheel::Event(EGEvent *pEvent)
{
  ...
  switch(Code){
    case EG_EVENT_REFR_EXT_DRAW_SIZE: {
      int32_t left = GetStylePadLeft(EG_PART_KNOB);
      int32_t right = GetStylePadRight(EG_PART_KNOB);
      int32_t top = GetStylePadTop(EG_PART_KNOB);
      int32_t bottom = GetStylePadBottom(EG_PART_KNOB);
      int32_t knob_pad = EG_MAX4(left, right, top, bottom) + 2;
    ...
    case EG_EVENT_PRESSING: {
      ...
      int32_t drag_limit = pInput->m_pDriver->m_ScrollLimit;
      ...
      int16_t Angle = 0;
      int32_t cir_w = GetStyleArcWidth(EG_PART_MAIN);
      int32_t r_in = Width / 2;
      Point.m_X -= r_in;
      Point.m_Y -= r_in;
      bool on_ring = true;
      r_in -= cir_w;
      if(r_in > EG_DPI_DEF / 2) {
        int32_t inner = cir_w / 2;
        r_in -= inner;
...
{
	EGDeviceContext *draw_ctx = pEvent->GetDeviceContext();
	int32_t Width = GetWidth();
	int32_t h = GetHeight();
	int32_t cx = m_Rect.GetX1() + Width / 2;
	int32_t cy = m_Rect.GetY1() + h / 2;
	int32_t Radius = Width / 2;

	EGDrawLine DrawLine;
	InititialseDrawLine(EG_PART_MAIN, &DrawLine);
	DrawLine.m_Width = (Radius * 628 / (256 / EG_CPICKER_DEF_QF)) / 100;
	DrawLine.m_Width += 2;
	uint16_t i;
	uint32_t a = 0;
	int32_t cir_w = GetStyleArcWidth(EG_PART_MAIN);
	...
	int32_t cir_w_extra = DrawLine.m_Width;
#else
	int32_t cir_w_extra = 0;
#endif
	for(i = 0; i <= 256; i += EG_CPICKER_DEF_QF, a += 360 * EG_CPICKER_DEF_QF) {
		DrawLine.m_Color = AngleToModeColorFast(i);
		uint16_t Angle = (uint16_t)(a >> 8); // i * 360 / 256 is the scale to apply, but we can skip multiplication here*/
		EGPoint Point[2];
		Point[0].m_X = cx + ((Radius + cir_w_extra) * EG_TrigoSin(Angle) >> EG_TRIGO_SHIFT);
		Point[0].m_Y = cy + ((Radius + cir_w_extra) * EG_TrigoCos(Angle) >> EG_TRIGO_SHIFT);
		Point[1].m_X = cx + ((Radius - cir_w - cir_w_extra) * EG_TrigoSin(Angle) >> EG_TRIGO_SHIFT);
		Point[1].m_Y = cy + ((Radius - cir_w - cir_w_extra) * EG_TrigoCos(Angle) >> EG_TRIGO_SHIFT);
		DrawLine.Draw(draw_ctx, &Point[0], &Point[1]);
	}
#if EG_DRAW_COMPLEX
	DrawMaskFreeParam(&MaskOutParam);
	DrawMaskFreeParam(&MaskInParam);
	DrawMaskRemoveID(MaskOutID);
	DrawMaskRemoveID(MaskInID);
#endif
}
...
void EGColorWheel::RefreshKnobPosition(void)
{
	InvalidateKnob();
	int32_t Width = GetWidth();
	int32_t scale_w = GetStyleArcWidth(EG_PART_MAIN);
	int32_t Radius = (Width - scale_w) / 2;
	uint16_t Angle = GetAngle();
	m_Knob.Position.m_X = (((int32_t)Radius * EG_TrigoSin(Angle)) >> EG_TRIGO_SHIFT);
	m_Knob.Position.m_Y = (((int32_t)Radius * EG_TrigoCos(Angle)) >> EG_TRIGO_SHIFT);
