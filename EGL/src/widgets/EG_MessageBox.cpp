...
#include "widgets/EG_MessageBox.h"
#if EG_USE_MSGBOX
...
#define EG_MSGBOX_FLAG_AUTO_PARENT EG_OBJ_FLAG_WIDGET_1 // Mark that the pParent was automatically created
...
bool AutoParent = false;
	...
	if(AutoParent) AddFlag(EG_MSGBOX_FLAG_AUTO_PARENT);
	...
	if(HasCloseButton) {
		...
		int32_t CloseSize = EG_FontGetLineHeight(pFont) + EG_DPX(10);
	...
	if(pButtonTexts) {
		...
		int32_t btn_h = EG_FontGetLineHeight(font) + EG_DPI_DEF / 10;
...
{
	if(HasFlagSet(EG_MSGBOX_FLAG_AUTO_PARENT))	EGObject::Delete(GetParent());
...
{
	if(HasFlagSet(EG_MSGBOX_FLAG_AUTO_PARENT))	DeleteAsync(GetParent());
