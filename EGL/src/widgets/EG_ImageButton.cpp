...
#include "widgets/EG_ImageButton.h"
...
void EGImageButton::DrawMain(EGEvent *pEvent)
{
	...
	int32_t tw = GetStyleTransformWidth(EG_PART_MAIN);
	int32_t th = GetStyleTransformHeight(EG_PART_MAIN);
	EGRect Rect(m_Rect);
	Rect.Inflate(tw, th);
	...
	if(pSource) {
		...
		DrawImage.Draw(pDC, &RectPart, pSource);
	}
	...
	if(pSource) {
		...
		bool comm_res = ClipCenter.Intersect(&ClipCenter, pDC->m_pClipRect);
		if(comm_res) {
			EGImageDecoder::GetInfo(pSource, &Header);
			const EGRect *pClipRect = pDC->m_pClipRect;
			pDC->m_pClipRect = &ClipCenter;
			...
			for(int32_t i = RectPart.GetX1(); i < (int32_t)(ClipCenter.GetX2()+ Header.Width - 1); i += Header.Width) {
				DrawImage.Draw(pDC, &RectPart, pSource);
