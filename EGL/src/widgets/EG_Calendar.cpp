...
void EGCalendar::DrawPartEventCB(EGEvent *pEvent)
{
	...
	EGEventDC *pDrawEvwnt = (EGEventDC*)pEvent->GetParam();
	if(pDrawEvwnt->m_Part == EG_PART_ITEMS) {
		if(pDrawEvwnt->m_Index < 7) {		// Day name styles
			pDrawEvwnt->m_pDrawRect->m_BackgroundOPA = EG_OPA_TRANSP;
			pDrawEvwnt->m_pDrawRect->m_BorderOPA = EG_OPA_TRANSP;
		}
		else if(pButtonMtrx->HasControl(pDrawEvwnt->m_Index, EG_BTNMATRIX_CTRL_DISABLED)) {
			pDrawEvwnt->m_pDrawRect->m_BackgroundOPA = EG_OPA_TRANSP;
			pDrawEvwnt->m_pDrawRect->m_BorderOPA = EG_OPA_TRANSP;
			pDrawEvwnt->m_pDrawLabel->m_Color = EG_MainPalette(EG_PALETTE_GREY);
		}
		if(pButtonMtrx->HasControl(pDrawEvwnt->m_Index, EG_CALENDAR_CTRL_HIGHLIGHT)) {
			pDrawEvwnt->m_pDrawRect->m_BackgroundOPA = EG_OPA_40;
			pDrawEvwnt->m_pDrawRect->m_BackgroundColor = EGTheme::GetColorPrimary(pButtonMtrx);
			if(pButtonMtrx->GetSelected() == pDrawEvwnt->m_Index) {
				pDrawEvwnt->m_pDrawRect->m_BackgroundOPA = EG_OPA_70;
			}
		}
		if(pButtonMtrx->HasControl(pDrawEvwnt->m_Index, EG_CALENDAR_CTRL_TODAY)) {
			pDrawEvwnt->m_pDrawRect->m_BorderOPA = EG_OPA_COVER;
			pDrawEvwnt->m_pDrawRect->m_BorderColor = EGTheme::GetColorPrimary(pButtonMtrx);
			pDrawEvwnt->m_pDrawRect->m_BorderWidth += 1;
		}
	}
}
