...
{
	int32_t tx = pTile->GetX();
	int32_t ty = pTile->GetY();
	m_pActiveTile = pTile;
...
void EGTileView::SetTileIndex(uint32_t ColumnIndex, uint32_t RowIndex, EG_AnimateEnable_e Enable)
{
	...
	int32_t tx = ColumnIndex * Width;
	int32_t ty = RowIndex * Height;

	uint32_t i;
	for(i = 0; i < GetChildCount(); i++) {
		EGTileViewTile *pTile = (EGTileViewTile*)GetChild(i);
		int32_t x = pTile->GetX();
		int32_t y = pTile->GetY();
		if(x == tx && y == ty) {
...
void EGTileView::EventCB(EGEvent *pEvent)
{
  EG_EventCode_e Code = pEvent->GetCode();
	...
	if(Code == EG_EVENT_SCROLL_END) {
		...
		int32_t Width = pTileView->GetContentWidth();
		int32_t Height = pTileView->GetContentHeight();
		EGPoint scroll_end;
		pTileView->GetScrollEnd(&scroll_end);
		int32_t left = scroll_end.m_X;
		int32_t top = scroll_end.m_Y;
		int32_t tx = ((left + (Width / 2)) / Width) * Width;
		int32_t ty = ((top + (Height / 2)) / Height) * Height;
