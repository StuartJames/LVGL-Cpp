...
int32_t    EGTabView::m_TabSizeCreate;
...
EGTabView::EGTabView(EGObject *pParent, EG_DirType_e TabPos, int32_t TabSize, 
...
void EGTabView::SetActiveTab(uint32_t Index, EG_AnimateEnable_e Enable)
{
	...
	if((m_TabPosition & EG_DIR_VER) != 0) {
		int32_t Gap = pContent->GetStylePadColumn(EG_PART_MAIN);
		int32_t Width = pContent->GetContentWidth();
		...
		int32_t Gap = pContent->GetStylePadRow(EG_PART_MAIN);
		int32_t Height = pContent->GetContentHeight();
