...
#include "widgets/EG_AnimatedImage.h"
#if EG_USE_ANIMIMG != 0

#if EG_USE_IMG == 0     // Testing of dependencies
#error "EGAnimatedImage: EGImage is required. Enable it in EG_Config.h (EG_USE_IMG  1) "
#endif
...
void EGAnimatedImage::SetSource(const void *pDescriptor[], uint8_t Count)
{
	m_pDicriptors = pDescriptor;
...
void EGAnimatedImage::IndexChange(EGAnimate *pAnimation, int32_t Index)
{
	...
	int32_t Idx = Index % pAnimateImage->m_PictureCount;
