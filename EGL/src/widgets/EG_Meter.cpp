...
EG_Indicator_t* EGMeter::AddNeedleImage(EG_MeterScale_t *pScale, const void *pSource, int32_t PivotX, int32_t PivotY)
{
  EG_Indicator_t *pIndicator = (EG_Indicator_t*)EG_AllocMem(sizeof(EG_Indicator_t));
	EG_ASSERT_MALLOC(pIndicator);
	m_IndicatorList.AddHead(pIndicator);
...
void EGMeter::Event(EGEvent *pEvent)
{
  EG_EventCode_e Code = pEvent->GetCode();
	if(Code == EG_EVENT_DRAW_MAIN) {
		...
		DrawArcs(pDC, &ScaleArea);
		DrawTicksLabels(pDC, &ScaleArea);
		DrawNeedles(pDC, &ScaleArea);
		int32_t EdgeRadius = ScaleArea.GetWidth() / 2;
		EGPoint ScaleCenter;
		...
		int32_t Width = GetStyleWidth(EG_PART_INDICATOR) / 2;
		int32_t Height = GetStyleHeight(EG_PART_INDICATOR) / 2;
		EGRect Rect;
...
void EGMeter::DrawArcs(EGDeviceContext *pDC, const EGRect *pScaleArea)
{
	EGDrawArc DrawArc;
	DrawArc.m_Rounded = GetStyleArcRounded(EG_PART_ITEMS);
	int32_t OutsideRadius = pScaleArea->GetWidth() / 2;
	EGPoint ScaleCenter;
	...
	EventDC.m_pDrawArc = &DrawArc;
	EventDC.m_Part = EG_PART_INDICATOR;
	EventDC.m_pClass = &c_MeterClass;
	EventDC.m_Type = EG_METER_DRAW_PART_ARC;
  POSITION Pos;
	for(pIndicator = (EG_Indicator_t*)m_IndicatorList.GetTail(Pos); pIndicator != NULL; pIndicator = (EG_Indicator_t*)m_IndicatorList.GetPrev(Pos)){
		...
		EventDC.m_Radius = OutsideRadius + pIndicator->TypeData.Arc.RadiusMod;
		EventDC.m_pSubPart = pIndicator;
		EventDC.m_pPoint1 = &ScaleCenter;
		EGEvent::EventSend(this, EG_EVENT_DRAW_PART_BEGIN, &EventDC);
		DrawArc.Draw(pDC, &ScaleCenter, EventDC.m_Radius, StartAngle, EndAngle);
...
void EGMeter::DrawTicksLabels(EGDeviceContext *pDC, const EGRect *pScaleArea)
{
	EGPoint CenterPoint;
	int32_t EdgeRadius = EG_MIN(pScaleArea->GetWidth() / 2, pScaleArea->GetHeight() / 2);
	...
	EventDC.m_Type = EG_METER_DRAW_PART_TICK;
  ...
  POSITION Pos;
	for(pScale = (EG_MeterScale_t*)m_ScaleList.GetTail(Pos); pScale != NULL; pScale = (EG_MeterScale_t*)m_ScaleList.GetPrev(Pos)){
		EventDC.m_pSubPart = pScale;
		int32_t OutsideRadius = EdgeRadius;
		int32_t r_in_minor = OutsideRadius - pScale->TickLength;
		int32_t r_in_major = OutsideRadius - pScale->TickMajorLength;
      ...
      POSITION Pos2;
		  for(pIndicator = (EG_Indicator_t*)m_IndicatorList.GetTail(Pos2); pIndicator != NULL; pIndicator = (EG_Indicator_t*)m_IndicatorList.GetPrev(Pos2)){
			...
			OuterPoint.PointTransform(UpscaleAngle, EGScale(256), &CenterPoint);
			EventDC.m_pPoint1 = &CenterPoint;
			EventDC.m_pPoint2 = &OuterPoint;
			EventDC.m_Index = i;
			EventDC.m_pDrawLabel = &label_dsc;
			if(DoMajor) {			// Draw the text
				DrawMaskRemoveID(OuterMaskID);
				uint32_t r_text = r_in_major - pScale->LabelGap;
				EGPoint Point;
				Point.m_X = CenterPoint.m_X + r_text;
				Point.m_Y = CenterPoint.m_Y;
				Point.PointTransform(UpscaleAngle, EGScale(256), &CenterPoint);
  			...
  			EGEvent::EventSend(this, EG_EVENT_DRAW_PART_BEGIN, &EventDC);
				EGSize LabelSize;
				EG_GetTextSize(&LabelSize, EventDC.m_pText, DrawLabel.m_pFont, DrawLabel.m_Kerning, DrawLabel.m_LineSpace,	EG_COORD_MAX, EG_TEXT_FLAG_NONE);
				EGRect LabelRect;
				LabelRect.SetX1(Point.m_X - LabelSize.m_X / 2);
				LabelRect.SetY1(Point.m_Y - LabelSize.m_Y / 2);
				LabelRect.SetX2(LabelRect.GetX1() + LabelSize.m_X);
				LabelRect.SetY2(LabelRect.GetY1() + LabelSize.m_Y);
				EventDC.m_pDrawLabel->Draw(pDC, &LabelRect, EventDC.m_pText, NULL);
				OuterMaskID = DrawMaskAdd(&OuterMask, NULL);
			}
			else {
				EventDC.m_pDrawLabel = NULL;
				EventDC.m_pText = NULL;
				EGEvent::EventSend(this, EG_EVENT_DRAW_PART_BEGIN, &EventDC);
			}
			InnerActMaskID = DrawMaskAdd(DoMajor ? &InnerMajorMask : &InnerMinorMask, NULL);
			DrawLine.Draw(pDC, &OuterPoint, &CenterPoint);
			DrawMaskRemoveID(InnerActMaskID);
			EGEvent::EventSend(this, EG_EVENT_DRAW_MAIN_END, &EventDC);
...
void EGMeter::DrawNeedles(EGDeviceContext *pDC, const EGRect *pScaleArea)
{
	int32_t EdgeRadius = pScaleArea->GetWidth() / 2;
	EGPoint ScaleCenter;
  ...
  POSITION Pos;
	for(pIndicator = (EG_Indicator_t*)m_IndicatorList.GetTail(Pos); pIndicator != NULL; pIndicator = (EG_Indicator_t*)m_IndicatorList.GetPrev(Pos)){
		...
		if(pIndicator->Type == EG_METER_INDICATOR_TYPE_NEEDLE_LINE) {
			...
			int32_t OutsideRadius = EdgeRadius + pScale->RadiusMod + pIndicator->TypeData.NeedleLine.RadiusMod;
			EGPoint EndPoint;
			EndPoint.m_Y = (EG_TrigoSin(Angle) * (OutsideRadius)) / EG_TRIGO_SIN_MAX + ScaleCenter.m_Y;
			EndPoint.m_X = (EG_TrigoCos(Angle) * (OutsideRadius)) / EG_TRIGO_SIN_MAX + ScaleCenter.m_X;
			...
			EventDC.m_Type = EG_METER_DRAW_PART_NEEDLE_LINE;
			EventDC.m_pDrawLine = &DrawLine;
			EventDC.m_pPoint2 = &EndPoint;
			EventDC.m_pPoint1 = &ScaleCenter;
			EGEvent::EventSend(this, EG_EVENT_DRAW_PART_BEGIN, &EventDC);
			DrawLine.Draw(pDC, EventDC.m_pPoint1, &EndPoint);
			EGEvent::EventSend(this, EG_EVENT_DRAW_PART_END, &EventDC);
		}
			...
			EventDC.m_Type = EG_METER_DRAW_PART_NEEDLE_IMG;
			EventDC.m_pDrawImage = &DrawImage;
			EGEvent::EventSend(this, EG_EVENT_DRAW_PART_BEGIN, &EventDC);
			DrawImage.Draw(pDC, &Rect, pIndicator->TypeData.NeedleImage.pSource);
			EGEvent::EventSend(this, EG_EVENT_DRAW_PART_END, &EventDC);
		}
	}
}
...
void EGMeter::InvArc(EG_Indicator_t *pIndicator, int32_t OldValue, int32_t NewValue)
{
	...
	EGRect ScaleArea;
	GetContentArea(&ScaleArea);
	int32_t OutsideRadius = ScaleArea.GetWidth() / 2;
	EGPoint ScaleCenter;
...
void EGMeter::InvLine(EG_Indicator_t *pIndicator, int32_t Value)
{
	...
	if(pIndicator->Type == EG_METER_INDICATOR_TYPE_NEEDLE_LINE) {
		...
		EGRect Rect2;
		EGImageBuffer::GetTransformedRect(&Rect2, info.Width, info.Height, Angle, EGScale(EG_SCALE_NONE), &pIndicator->TypeData.NeedleImage.Pivot);
