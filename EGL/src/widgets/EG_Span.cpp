...
void EGSpanGroup::SetIndent(int32_t Indent)
{
...
void EGSpanGroup::RefreshMode(void)
{
	if(m_Mode == EG_SPAN_MODE_EXPAND) {
		...
		if(GetStyleHeight(EG_PART_MAIN) == EG_SIZE_CONTENT) {
			...
			int32_t Height = GetExpandHeight(Width);
...
int32_t EGSpanGroup::GetMaxLineHeight(void)
{
...
int32_t EGSpanGroup::GetExpandHeight(int32_t Width)
{
POSITION Pos;
	...
	int32_t MaxWidth = Width;
	int32_t Indent = ConvertIndentPCT(MaxWidth);
	int32_t MaxW = MaxWidth - Indent; //  first line need minus indent 
	...
	while(pSpan) {
		int SnippetCount = 0;
		int32_t MaxLineHeight = 0; //  the max height of span-font when a line have a lot of span 
...
void EGSpanGroup::Event(EGEvent *pEvent)
{
  ...
  switch(Code){
    ...
    case EG_EVENT_GET_SELF_SIZE: {
      ...
      if(m_Mode == EG_SPAN_MODE_EXPAND) {
        if(m_Refresh) {
          m_CacheWidth = (int32_t)GetExpandWidth(0);
...
bool EGSpanGroup::GetSnippetInfo(const char *txt, const EG_Font_t *pFont,
															 int32_t Kerning, int32_t MaxWidth, EG_TextFlag_t flag,
															 int32_t *UseWidth, uint32_t *end_ofs)
{
	if(txt == NULL || txt[0] == '\0') {
		*end_ofs = 0;
		*UseWidth = 0;
...
int32_t EGSpanGroup::GetSpanStyleKerning(EG_Span_t *pSpan)
{
int32_t Kerning;
EG_StyleValue_t Value;
	...
	else Kerning = (int32_t)Value.Number;
	return Kerning;
}
...
int32_t EGSpanGroup::ConvertIndentPCT(int32_t Width)
{
	int32_t Indent = m_Indent;
...
POSITION Pos;
	...
	EGRect ClipRect;
	if(!ClipRect.Intersect(&Rect, pDC->m_pClipRect)) return; //  return if no draw area 
	const EGRect *clip_area_ori = pDC->m_pClipRect;
	pDC->m_pClipRect = &ClipRect;
	EG_TextFlag_t TextFlag = EG_TEXT_FLAG_NONE;	//  init draw variable 
	int32_t LineSpace = GetStyleTextLineSpace(EG_PART_MAIN);
	int32_t MaxWidth = Rect.GetWidth();
	int32_t Indent = ConvertIndentPCT(MaxWidth);
	int32_t MaxW = MaxWidth - Indent; //  first line need minus indent 
	...
	while(pSpan) {
		...
		int32_t MaxLineHeight = 0;   //  the max height of span-font when a line have a lot of pSpan 
		int32_t max_baseline = 0; // baseline of the highest pSpan
		ClearSnippet();
		while(1) {		//  the loop control to find a line and push the relevant pSpan info into stack  
			if(pText[CurrentOffset] == '\0') {			//  switch to the next pSpan when current is end 
				pSpan = (EG_Span_t*)m_SpanList.GetNext(Pos);
			...
			if(isfill) {
				if(NextOffset > 0 && GetSnippetCount() > 0) {
					...
					//  This phenomenon should be avoided when GetSnippetCount() > 0 
		...
		{		//  Whether the current line is the end line and does overflow processing 
			...
			int32_t NextLineHeight = pLastSnippet->LineHeight;
		...
		}
	{
		...
		if(align == EG_TEXT_ALIGN_CENTER || align == EG_TEXT_ALIGN_RIGHT) {
			int32_t align_ofs = 0;
			int32_t txts_w = IsFirstLine ? Indent : 0;
		...
		for(int i = 0; i < item_cnt; i++) {
			...
			int32_t ellipsis_width = Rect.GetX1() + MaxWidth - dot_width;
			...
			while(j < txt_bytes) {
				...
				if(ellipsis_valid && pos.m_X + ChrWidth + pSnippet->Kerning > ellipsis_width) {
					...
					DrawLabel.DrawChar(pDC, &pos, Chr);
					if(ChrWidth > 0) {
						pos.m_X = pos.m_X + ChrWidth + pSnippet->Kerning;
					}
				}
