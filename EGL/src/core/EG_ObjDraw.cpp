...
int32_t EGObject::CalculateExtDrawSize(uint32_t Part)
{
int32_t Size = 0;

	int32_t ShadowWidth = GetStyleShadowWidth(Part);
	if(ShadowWidth) {
	...
	int32_t OutlineWidth = GetStyleOutlineWidth(Part);
	if(OutlineWidth) {
		if(GetStyleOutlineOPA(Part) > EG_OPA_MIN) {
			Size = EG_MAX(Size, GetStyleOutlinePadding(Part) + OutlineWidth);
		}
	}
	int32_t WidthHeight = EG_MAX(GetStyleTransformWidth(Part), GetStyleTransformHeight(Part));
...
bool EGObject::DrawPartCheckType(EGEventDC *pDrawEvent, const EG_ClassType_t *pClass, uint32_t Type)
{
	if(pDrawEvent->m_pClass == pClass && pDrawEvent->m_Type == Type) return true;
	else return false;
}
...
{
	int32_t SizeOld = GetExtDrawSize();
	int32_t SizeNew = 0;
...
int32_t EGObject::GetExtDrawSize(void) const
{
