	...
	if(m_pActiveObj) {	// Forget the act obj and send a released Call the ancestor's event handler
		...
		if(pScrollObject) {
			...
			if(Angle != 0 || Zoom != EG_SCALE_NONE) {
				Angle = -Angle;
				Zoom = (256 * 256) / Zoom;
				pProcess->Pointer.ScrollThrowVector.PointTransform(Angle, EGScale(Zoom), &Pivot);
				pProcess->Pointer.LastScrollThrowVector.PointTransform(Angle, EGScale(Zoom), &Pivot);
			}
		}
	}
