...
EG_StyleTransitionDescriptor_t EGDefTheme::m_TransDelayed;
EG_StyleTransitionDescriptor_t EGDefTheme::m_TransNormal;
...
static const EGStyleProperty_e TransitionProperties[] = {
  ...
  if(m_Flags & MODE_DARK){
...
#if TRANSITION_TIME
	InitialiseTransitionDescriptor(&m_TransDelayed, TransitionProperties, EGAnimate::PathLinear, TRANSITION_TIME, 70, NULL);
	InitialiseTransitionDescriptor(&m_TransNormal, TransitionProperties, EGAnimate::PathLinear, TRANSITION_TIME, 0, NULL);
	...
	m_Pressed.SetColorFilterDescriptor(&m_DarkFilter);
	m_Pressed.SetColorFilterOPA(35);

	StyleInitialiseReset(&m_Disabled);
	m_Disabled.SetColorFilterDescriptor(&m_GreyFilter);
