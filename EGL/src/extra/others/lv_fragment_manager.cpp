...
static lv_fragment_managed_states_t *fragment_attach(lv_fragment_manager_t *manager, lv_fragment_t *fragment,
																										 EGObject *const *container);


lv_fragment_manager_t *lv_fragment_manager_create(lv_fragment_t *parent)
{
	lv_fragment_manager_t *instance = EG_AllocMem(sizeof(lv_fragment_manager_t));
	EG_ZeroMem(instance, sizeof(lv_fragment_manager_t));
...
void lv_fragment_manager_add(lv_fragment_manager_t *manager, lv_fragment_t *fragment, EGObject *const *container)
{
...
void lv_fragment_manager_remove(lv_fragment_manager_t *manager, lv_fragment_t *fragment)
{
	...
	EG_ASSERT(fragment->managed->manager == manager);
...
void lv_fragment_manager_push(lv_fragment_manager_t *manager, lv_fragment_t *fragment, EGObject *const *container)
{
	lv_fragment_stack_item_t *top = _lv_ll_get_tail(&manager->stack);
	if(top != NULL) {
	...
	EG_ZeroMem(item, sizeof(lv_fragment_stack_item_t));
	item->states = states;
	item_create_obj(states);
}
...
lv_fragment_t *lv_fragment_manager_find_by_container(lv_fragment_manager_t *manager, const EGObject *container)
{
	EG_ASSERT(manager);
	lv_fragment_managed_states_t *states;
	_LV_LL_READ(&manager->attached, states)
																										 ...
																										 EGObject *const *container)
{
	EG_ASSERT(manager);
	EG_ASSERT(fragment);
	EG_ASSERT(fragment->managed == NULL);
	lv_fragment_managed_states_t *states = _lv_ll_ins_tail(&manager->attached);
	EG_ZeroMem(states, sizeof(lv_fragment_managed_states_t));
