...
static void obj_delete_event_cb(EGEvent *e);

static lv_ll_t subs_ll;

lv_event_code_t EG_EVENT_MSG_RECEIVED;

void lv_msg_init(void)
{
	EG_EVENT_MSG_RECEIVED = lv_event_register_id();
...
void *lv_msg_subsribe_obj(uint32_t msg_id, EGObject *obj, void *user_data)
{
	...
	if(s_first == NULL) {
		lv_obj_add_event_cb(obj, obj_delete_event_cb, EG_EVENT_DELETE, s);
	}
	return s;
}
...
uint32_t lv_msg_unsubscribe_obj(uint32_t msg_id, EGObject *obj)
{
	uint32_t cnt = 0;
...
lv_msg_t *lv_event_get_msg(EGEvent *e)
{
	if(e->code == EG_EVENT_MSG_RECEIVED) {
...
{
	EG_UNUSED(s);
	lv_event_send(m->_priv_data, EG_EVENT_MSG_RECEIVED, m);
}
