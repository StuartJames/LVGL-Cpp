...
#include "extra/libs/lv_rlottie.h"
#if EG_USE_RLOTTIE
...
static void lv_rlottie_constructor(const EG_ClassType_t *class_p, EGObject *obj);
static void lv_rlottie_destructor(const EG_ClassType_t *class_p, EGObject *obj);
static void next_frame_task_cb(EGTimer *t);

const EG_ClassType_t lv_rlottie_class = {
	.constructor_cb = lv_rlottie_constructor,
	.destructor_cb = lv_rlottie_destructor,
	.base_class = &lv_img_class};

static EG_Coord_t create_width;
static EG_Coord_t create_height;
...
EGObject *lv_rlottie_create_from_raw(EGObject *parent, EG_Coord_t width, EG_Coord_t height, const char *rlottie_desc)
{
	create_width = width;
	create_height = height;
...
void lv_rlottie_set_play_mode(EGObject *obj, const lv_rlottie_ctrl_t ctrl)
{
...
void lv_rlottie_set_current_frame(EGObject *obj, const size_t goto_frame)
{
	lv_rlottie_t *rlottie = (lv_rlottie_t *)obj;
...
static void lv_rlottie_constructor(const EG_ClassType_t *class_p, EGObject *obj)
{
	...
	rlottie->allocated_buf = EG_AllocMem(allocaled_buf_size);
...
static void lv_rlottie_destructor(const EG_ClassType_t *class_p, EGObject *obj)
{
	...
	if(rlottie->allocated_buf) {
		EG_FreeMem(rlottie->allocated_buf);
...
static void convert_to_rgba5658(uint32_t *pix, const size_t width, const size_t height)
{
	...
	for(size_t y = 0; y < height; y++) {
		...
		for(size_t x = 0; x < width; x++) {
			...
			EG_CopyMem(dest, &r, sizeof(r));
			dest[sizeof(r)] = (uint8_t)(in >> 24);
			dest += EG_IMG_PX_SIZE_ALPHA_BYTE;
		}
		src += width;
	}
}
		...
		if((rlottie->play_ctrl & LV_RLOTTIE_CTRL_BACKWARD) == LV_RLOTTIE_CTRL_BACKWARD) {
			if(rlottie->current_frame > 0)
				...
				if((rlottie->play_ctrl & LV_RLOTTIE_CTRL_LOOP) == LV_RLOTTIE_CTRL_LOOP)
					...
					lv_event_send(obj, EG_EVENT_READY, NULL);
					lv_timer_pause(t);
					return;
				}
			}
