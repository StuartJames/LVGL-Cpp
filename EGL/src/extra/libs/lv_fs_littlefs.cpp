...
void lv_fs_littlefs_init(void)
{
	...
	/*Set up fields...*/
	fs_drv.letter = EG_FS_LITTLEFS_LETTER;
	fs_drv.cache_size = EG_FS_LITTLEFS_CACHE_SIZE;
...
{
	EG_FS_Driver_t *drv_p = lv_fs_get_drv(label);
...
static void *fs_open(EG_FS_Driver_t *drv, const char *path, EG_FS_Mode_e mode)
{
	...
	lfs_file_t *file_p = EG_AllocMem(sizeof(lfs_file_t));
...
static EG_FSResult_e fs_write(EG_FS_Driver_t *drv, void *file_p, const void *buf, uint32_t btw, uint32_t *bw)
{
	...
	if(result < 0 || lfs_file_sync(lfs_p, file_p) < 0) return EG_FS_RES_UNKNOWN;
...
static void *fs_dir_open(EG_FS_Driver_t *drv, const char *path)
{
	...
	lfs_dir_t *dir_p = EG_AllocMem(sizeof(lfs_dir_t));
 ...
 * @param drv       pointer to a driver where this function belongs
 * @param rddir_p   pointer to an initialized 'EG_FS_Dir_t' variable
 * @param fn        pointer to a buffer to store the filename
 * @return          EG_FS_RES_OK: no error or  any error from @EG_FSResult_e enum
 */
static EG_FSResult_e fs_dir_read(EG_FS_Driver_t *drv, void *rddir_p, char *fn)
{
	lfs_t *lfs_p = drv->user_data;
 ...
 * @return          EG_FS_RES_OK: no error or  any error from @EG_FSResult_e enum
 */
static EG_FSResult_e fs_dir_close(EG_FS_Driver_t *drv, void *rddir_p)
{
	lfs_t *lfs_p = drv->user_data;
