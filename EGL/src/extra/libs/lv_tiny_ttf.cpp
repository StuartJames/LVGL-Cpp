...
#define STBTT_malloc(x, u) ((void)(u), EG_AllocMem(x))
#define STBTT_free(x, u) ((void)(u), EG_FreeMem(x))
#define TTF_MALLOC(x) (EG_AllocMem(x))
#define TTF_FREE(x) (EG_FreeMem(x))
...
static void ttf_cb_stream_read(ttf_cb_stream_t *stream, void *data, size_t to_read)
{
	if(stream->file != NULL) {
		...
		EG_CopyMem(data, ((const unsigned char *)stream->data + stream->position), to_read);
...
{
	if(stream->file != NULL) {
		lv_fs_seek(stream->file, position, EG_FS_SEEK_SET);
	}
	else {
																 ...
																 uint32_t unicode_letter_next)
{
	if(unicode_letter < 0x20 ||
		 unicode_letter == 0xf8ff || /*EG_SYMBOL_DUMMY*/
		 unicode_letter == 0x200c) { /*ZERO WIDTH NON-JOINER*/
		dsc_out->BoxWidth = 0;
		dsc_out->AdvWidth = 0;
		dsc_out->BoxHeight = 0; /*height of the bitmap in [px]*/
		dsc_out->OffsetX = 0; /*X offset of the bitmap in [pf]*/
		dsc_out->OffsetY = 0; /*Y offset of the bitmap in [pf]*/
	...
	dsc_out->AdvWidth = (uint16_t)floor((((float)advw + (float)k) * dsc->scale) +
																	 0.5f); /*Horizontal space required by the glyph in [px]*/

	dsc_out->AdvWidth = (uint16_t)floor((((float)advw + (float)k) * dsc->scale) +
																	 0.5f); /*Horizontal space required by the glyph in [px]*/
	dsc_out->BoxWidth = (x2 - x1 + 1);         /*width of the bitmap in [px]*/
	dsc_out->BoxHeight = (y2 - y1 + 1);         /*height of the bitmap in [px]*/
	dsc_out->OffsetX = x1;                    /*X offset of the bitmap in [pf]*/
	dsc_out->OffsetY = -y2;                   /*Y offset of the bitmap measured from the as line*/
	dsc_out->BitsPerPixel = 8;                       /*Bits per pixel: 1/2/4/8*/
	dsc_out->IsPlaceholder = false;
	return true; /*true: glyph found; false: glyph was not found*/
}

static const uint8_t *ttf_get_glyph_bitmap_cb(const EG_Font_t *font, uint32_t unicode_letter)
{
	ttf_font_desc_t *dsc = (ttf_font_desc_t *)font->dsc;
	...
	EG_SetMem(&cache_key, 0, sizeof(cache_key)); /*Zero padding*/
	cache_key.unicode_letter = unicode_letter;
	cache_key.LineHeight = font->LineHeight;
	uint8_t *buffer = NULL;
	EG_LRUGet(dsc->bitmap_cache, &cache_key, sizeof(cache_key), (void **)&buffer);
	if(buffer) {
		return buffer;
	}
	EG_LOG_TRACE("cache miss for letter: %u", unicode_letter);
	/*Prepare space in cache*/
	size_t szb = h * stride;
	buffer = EG_AllocMem(szb);
	if(!buffer) {
		EG_LOG_ERROR("failed to allocate cache value");
		return NULL;
	}
	EG_SetMem(buffer, 0, szb);
	if(EG_LRU_OK != EG_LRUSet(dsc->bitmap_cache, &cache_key, sizeof(cache_key), buffer, szb)) {
		EG_LOG_ERROR("failed to add cache value");
		EG_FreeMem(buffer);
		return NULL;
	}
	/*Render into cache*/
																		 ...
																		 size_t cache_size)
{
	if((path == NULL && data == NULL) || 0 >= font_size) {
		EG_LOG_ERROR("tiny_ttf: invalid argument\n");
		return NULL;
	}
	ttf_font_desc_t *dsc = (ttf_font_desc_t *)TTF_MALLOC(sizeof(ttf_font_desc_t));
	if(dsc == NULL) {
		EG_LOG_ERROR("tiny_ttf: out of memory\n");
		return NULL;
	}
#if EG_TINY_TTF_FILE_SUPPORT
	if(path != NULL) {
		if(EG_FS_RES_OK != lv_fs_open(&dsc->file, path, EG_FS_MODE_RD)) {
			EG_LOG_ERROR("tiny_ttf: unable to open %s\n", path);
			goto err_after_dsc;
		}
	...
	if(0 == stbtt_InitFont(&dsc->info, &dsc->stream, stbtt_GetFontOffsetForIndex(&dsc->stream, 0))) {
		EG_LOG_ERROR("tiny_ttf: init failed\n");
		goto err_after_dsc;
	}
	...
	dsc->bitmap_cache = EG_LRUCreate(cache_size, font_size * font_size, EG_FreeMem, EG_FreeMem);
	if(dsc->bitmap_cache == NULL) {
		EG_LOG_ERROR("failed to create lru cache");
		goto err_after_dsc;
	}

	EG_Font_t *out_font = (EG_Font_t *)TTF_MALLOC(sizeof(EG_Font_t));
	if(out_font == NULL) {
		EG_LOG_ERROR("tiny_ttf: out of memory\n");
		goto err_after_bitmap_cache;
	}
	EG_SetMem(out_font, 0, sizeof(EG_Font_t));
	out_font->GetGlyphPropsCB = ttf_get_glyph_dsc_cb;
	out_font->GetGlyphBitmapCB = ttf_get_glyph_bitmap_cb;
	out_font->dsc = dsc;
	lv_tiny_ttf_set_size(out_font, font_size);
	return out_font;
err_after_bitmap_cache:
	EG_LRUDelete(dsc->bitmap_cache);
err_after_dsc:
	TTF_FREE(dsc);
	return NULL;
}
...
#endif
			EG_LRUDelete(ttf->bitmap_cache);
			TTF_FREE(ttf);
		}
		TTF_FREE(font);
	}
}
#endif /*EG_USE_TINY_TTF*/
