...
static gd_GIF *gif_open(gd_GIF *gif_base)
{
	...
	/* Presence of GCT */
	if(!(fdsz & 0x80)) {
		EG_LOG_WARN("no global color table\n");
		goto fail;
	}
	/* Color Space's Depth */
	depth = ((fdsz >> 4) & 7) + 1;
...
/* Create gd_GIF Structure. */
	...
	gif = EG_AllocMem(sizeof(gd_GIF) + 5 * width * height);
#elif EG_COLOR_DEPTH == 16
	gif = EG_AllocMem(sizeof(gd_GIF) + 4 * width * height);
#elif EG_COLOR_DEPTH == 8 || EG_COLOR_DEPTH == 1
	gif = EG_AllocMem(sizeof(gd_GIF) + 3 * width * height);
#endif
...
#elif EG_COLOR_DEPTH == 16
		EG_Color_t c = EG_MixColor(*(bgcolor + 0), *(bgcolor + 1), *(bgcolor + 2));
	...
	gif->anim_start = f_gif_seek(gif, 0, EG_FS_SEEK_CUR);
	gif->loop_count = -1;
	goto ok;
fail:
...
discard_sub_blocks(gd_GIF *gif)
{
	...
	do {
		...
		f_gif_seek(gif, size, EG_FS_SEEK_CUR);
	} while(size);
}
...
read_plain_text_ext(gd_GIF *gif)
{
	if(gif->plain_text) {
		...
		uint8_t cw, ch, fg, bg;
		size_t sub_block;
		f_gif_seek(gif, 1, EG_FS_SEEK_CUR); /* block size = 12 */
		tx = read_num(gif);
		ty = read_num(gif);
		tw = read_num(gif);
		th = read_num(gif);
		...
		f_gif_read(gif, &fg, 1);
		f_gif_read(gif, &bg, 1);
		sub_block = f_gif_seek(gif, 0, EG_FS_SEEK_CUR);
		gif->plain_text(gif, tx, ty, tw, th, cw, ch, fg, bg);
		f_gif_seek(gif, sub_block, EG_FS_SEEK_SET);
	}
	else {
		/* Discard plain text metadata. */
		f_gif_seek(gif, 13, EG_FS_SEEK_CUR);
	}
...
read_graphic_control_ext(gd_GIF *gif)
{
	...
	f_gif_seek(gif, 1, EG_FS_SEEK_CUR);
...
{
	if(gif->comment) {
		size_t sub_block = f_gif_seek(gif, 0, EG_FS_SEEK_CUR);
		gif->comment(gif);
		f_gif_seek(gif, sub_block, EG_FS_SEEK_SET);
	}
...
read_application_ext(gd_GIF *gif)
{
	...
	if(!strncmp(app_id, "NETSCAPE", sizeof(app_id))) {
		...
		f_gif_seek(gif, 2, EG_FS_SEEK_CUR);
...
new_table(int key_size)
{
	...
	Table *table = EG_AllocMem(sizeof(*table) + sizeof(Entry) * init_bulk);
	if(table) {
...
add_entry(Table **tablep, uint16_t length, uint16_t prefix, uint8_t suffix)
{
	...
	if(table->nentries == table->bulk) {
		table->bulk *= 2;
		table = EG_ReallocMem(table, sizeof(*table) + sizeof(Entry) * table->bulk);
		if(!table) return -1;
		table->entries = (Entry *)&table[1];
		*tablep = table;
	}
...
read_image_data(gd_GIF *gif, int interlace)
{
	...
	f_gif_read(gif, &byte, 1);
	key_size = (int)byte;
	start = f_gif_seek(gif, 0, EG_FS_SEEK_CUR);
	discard_sub_blocks(gif);
	end = f_gif_seek(gif, 0, EG_FS_SEEK_CUR);
	f_gif_seek(gif, start, EG_FS_SEEK_SET);
	clear = 1 << key_size;
	stop = clear + 1;
	...
	f_gif_seek(gif, end, EG_FS_SEEK_SET);
	return 0;
}
	...
	for(j = 0; j < gif->fh; j++) {
		for(k = 0; k < gif->fw; k++) {
			...
			if(!gif->gce.transparency || index != gif->gce.tindex) {
				...
				buffer[(i + k) * 4 + 3] = 0xFF;
#elif EG_COLOR_DEPTH == 16
				EG_Color_t c = EG_MixColor(*(color + 0), *(color + 1), *(color + 2));
...
int gd_get_frame(gd_GIF *gif)
{
	...
	while(sep != ',') {
		if(sep == ';') {
			f_gif_seek(gif, gif->anim_start, EG_FS_SEEK_SET);
...
static bool f_gif_open(gd_GIF *gif, const void *path, bool is_file)
{
	...
	if(is_file) {
		EG_FSResult_e res = lv_fs_open(&gif->fd, path, EG_FS_MODE_RD);
		if(res != EG_FS_RES_OK)
			return false;
		else
			return true;
