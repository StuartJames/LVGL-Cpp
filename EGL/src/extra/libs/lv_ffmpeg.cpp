...
#include "extra/libs/lv_ffmpeg.h"
#if EG_USE_FFMPEG != 0
...
static EG_Result_t decoder_info(lv_img_decoder_t *decoder, const void *src, EG_ImageHeader_t *header);
static EG_Result_t decoder_open(lv_img_decoder_t *dec, ImageDecoderDescriptor_t *dsc);
static void decoder_close(lv_img_decoder_t *dec, ImageDecoderDescriptor_t *dsc);
...
static int ffmpeg_get_img_header(const char *path, EG_ImageHeader_t *header);
...
static void lv_ffmpeg_player_constructor(const EG_ClassType_t *class_p, EGObject *obj);
static void lv_ffmpeg_player_destructor(const EG_ClassType_t *class_p, EGObject *obj);

#if EG_COLOR_DEPTH != 32
static void convert_color_depth(uint8_t *img, uint32_t px_cnt);
#endif

const EG_ClassType_t lv_ffmpeg_player_class = {
...
EG_Result_t lv_ffmpeg_player_set_src(EGObject *obj, const char *path)
{
	...
	if(!player->ffmpeg_ctx) {
		EG_LOG_ERROR("ffmpeg file open failed: %s", path);
		goto failed;
	}
	...
	if(has_alpha) {
		data_size = width * height * EG_IMG_PX_SIZE_ALPHA_BYTE;
	}
	else {
		data_size = width * height * EG_COLOR_SIZE / 8;
	}
	...
	player->imgdsc.header.cf = has_alpha ? EG_IMG_CF_TRUE_COLOR_ALPHA : EG_IMG_CF_TRUE_COLOR;
	...
	if(period > 0) {
		EG_LOG_INFO("frame refresh period = %d ms, rate = %d fps",
								period, 1000 / period);
		lv_timer_set_period(player->timer, period);
	}
	else {
		EG_LOG_WARN("unable to get frame refresh period");
	}
...
void lv_ffmpeg_player_set_cmd(EGObject *obj, lv_ffmpeg_player_cmd_t cmd)
{
	...
	if(!player->ffmpeg_ctx) {
		EG_LOG_ERROR("ffmpeg_ctx is NULL");
		return;
	}

	EGTimer *timer = player->timer;
...
{
	/* Get the source type */
	EG_ImageSource_t src_type = lv_img_src_get_type(src);
...
static EG_Result_t decoder_open(lv_img_decoder_t *decoder, ImageDecoderDescriptor_t *dsc)
{
	if(dsc->src_type == EG_IMG_SRC_FILE) {
		...
		if(ffmpeg_update_next_frame(ffmpeg_ctx) < 0) {
			ffmpeg_close(ffmpeg_ctx);
			EG_LOG_ERROR("ffmpeg update frame failed");
			return EG_RES_INVALID;
		}
...
static void decoder_close(lv_img_decoder_t *decoder, ImageDecoderDescriptor_t *dsc)
{
...
static void convert_color_depth(uint8_t *img, uint32_t px_cnt)
{
	EG_Color32_t *img_src_p = (EG_Color32_t *)img;
	struct lv_img_pixel_color_s *img_dst_p = (struct lv_img_pixel_color_s *)img;

	for(uint32_t i = 0; i < px_cnt; i++) {
		EG_Color32_t temp = *img_src_p;
		img_dst_p->c = EG_ColorHex(temp.full);
...
static int ffmpeg_output_video_frame(struct ffmpeg_context_s *ffmpeg_ctx)
{
	...
	if(frame->width != width || frame->height != height || frame->format != ffmpeg_ctx->video_dec_ctx->pix_fmt) {
	...
	EG_LOG_TRACE("video_frame coded_n:%d", frame->coded_picture_number);
	...
	if(ffmpeg_ctx->sws_ctx == NULL) {
		...
		if(ffmpeg_pix_fmt_is_yuv(ffmpeg_ctx->video_dec_ctx->pix_fmt)) {
			...
			if((width & 0x7) || (height & 0x7)) {
				EG_LOG_WARN("The width(%d) and height(%d) the image "
	...
	if(!ffmpeg_ctx->has_alpha) {
		...
		if(dst_linesize != lv_linesize) {
			EG_LOG_WARN("ffmpeg linesize = %d, but lvgl image require %d",
									dst_linesize,
									lv_linesize);
																...
																struct ffmpeg_context_s *ffmpeg_ctx)
{
	...
	if(ret < 0) {
		EG_LOG_ERROR("Error submitting a packet for decoding (%s)",
								 av_err2str(ret));
		return ret;
	}
	...
	while(ret >= 0) {
		...
		if(ret < 0) {
			...
			EG_LOG_ERROR("Error during decoding (%s)", av_err2str(ret));
			return ret;
		}
		...
		if(ret < 0) {
			EG_LOG_WARN("ffmpeg_decode_packet ended %d", ret);
			return ret;
		}
	}
																		 ...
																		 enum AVMediaType type)
{
	...
	if(ret < 0) {
		EG_LOG_ERROR("Could not find %s stream in input file",
		...
		if(dec == NULL) {
			EG_LOG_ERROR("Failed to find %s codec",
									 av_get_media_type_string(type));
		...
		*dec_ctx = avcodec_alloc_context3(dec);
		if(*dec_ctx == NULL) {
			EG_LOG_ERROR("Failed to allocate the %s codec context",
									 av_get_media_type_string(type));
		...
		if((ret = avcodec_open2(*dec_ctx, dec, &opts)) < 0) {
			EG_LOG_ERROR("Failed to open %s codec",
									 av_get_media_type_string(type));
			return ret;
		}
																 ...
																 EG_ImageHeader_t *header)
{
	...
	if(avformat_open_input(&fmt_ctx, filepath, NULL, NULL) < 0) {
		EG_LOG_ERROR("Could not open source file %s", filepath);
		goto failed;
	}
	...
	if(ffmpeg_open_codec_context(&video_stream_idx, &video_dec_ctx,
		...
		header->cf = (has_alpha ? EG_IMG_CF_TRUE_COLOR_ALPHA : EG_IMG_CF_TRUE_COLOR);
...
static int ffmpeg_get_frame_refr_period(struct ffmpeg_context_s *ffmpeg_ctx)
{
	...
	if(avg_frame_rate_num > 0) {
		int period = 1000 * (int64_t)ffmpeg_ctx->video_stream->avg_frame_rate.den / avg_frame_rate_num;
		return period;
	}
...
static int ffmpeg_update_next_frame(struct ffmpeg_context_s *ffmpeg_ctx)
{
	...
	while(1) {
		...
		if(av_read_frame(ffmpeg_ctx->fmt_ctx, &(ffmpeg_ctx->pkt)) >= 0) {
			...
			if(ret < 0) {
				EG_LOG_WARN("video frame is empty %d", ret);
				break;
			}
...
struct ffmpeg_context_s *ffmpeg_open_file(const char *path)
{
	...
	if(ffmpeg_ctx == NULL) {
		EG_LOG_ERROR("ffmpeg_ctx malloc failed");
		goto failed;
	}
...
static int ffmpeg_image_allocate(struct ffmpeg_context_s *ffmpeg_ctx)
{
	...
	EG_LOG_INFO("alloc video_src_bufsize = %d", ret);
	...
	EG_LOG_INFO("allocate video_dst_bufsize = %d", ret);

	ffmpeg_ctx->frame = av_frame_alloc();

	if(ffmpeg_ctx->frame == NULL) {
		EG_LOG_ERROR("Could not allocate frame");
		return -1;
	}
...
{
	if(ffmpeg_ctx == NULL) {
		EG_LOG_WARN("ffmpeg_ctx is NULL");
		return;
	}
	...
	EG_LOG_INFO("ffmpeg_ctx closed");
}

static void lv_ffmpeg_player_frame_update_cb(EGTimer *timer)
{
	EGObject *obj = (EGObject *)timer->user_data;
	lv_ffmpeg_player_t *player = (lv_ffmpeg_player_t *)obj;
	...
	if(has_next < 0) {
		lv_ffmpeg_player_set_cmd(obj, player->auto_restart ? EG_FFMPEG_PLAYER_CMD_START : EG_FFMPEG_PLAYER_CMD_STOP);
		return;
	}
...
static void lv_ffmpeg_player_constructor(const EG_ClassType_t *class_p,
																				 EGObject *obj)
{
...
static void lv_ffmpeg_player_destructor(const EG_ClassType_t *class_p,
																				EGObject *obj)
{
	EG_TRACE_OBJ_CREATE("begin");
