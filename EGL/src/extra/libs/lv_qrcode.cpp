...
#include "extra/libs/lv_qrcode.h"
#if EG_USE_QRCODE
...
static void lv_qrcode_constructor(const EG_ClassType_t *class_p, EGObject *obj);
static void lv_qrcode_destructor(const EG_ClassType_t *class_p, EGObject *obj);

const EG_ClassType_t lv_qrcode_class = {
	.constructor_cb = lv_qrcode_constructor,
	.destructor_cb = lv_qrcode_destructor,
	.base_class = &lv_canvas_class};

static EG_Coord_t size_param;
static EG_Color_t dark_color_param;
static EG_Color_t light_color_param;
...
EG_Result_t lv_qrcode_update(EGObject *qrcode, const void *data, uint32_t data_len)
{
	EG_Color_t c;
	c.full = 1;
	lv_canvas_fill_bg(qrcode, c, EG_OPA_COVER);

	if(data_len > qrcodegen_BUFFER_LEN_MAX) return EG_RES_INVALID;
	...
	uint8_t *qr0 = EG_AllocMem(qrcodegen_BUFFER_LEN_FOR_VERSION(qr_version));
	EG_ASSERT_MALLOC(qr0);
	uint8_t *data_tmp = EG_AllocMem(qrcodegen_BUFFER_LEN_FOR_VERSION(qr_version));
	EG_ASSERT_MALLOC(data_tmp);
	EG_CopyMem(data_tmp, data, data_len);
	...
	EG_Coord_t obj_w = imgdsc->header.w;
...
static void lv_qrcode_constructor(const EG_ClassType_t *class_p, EGObject *obj)
{
	...
	uint32_t buf_size = EG_CANVAS_BUF_SIZE_INDEXED_1BIT(size_param, size_param);
	uint8_t *buf = EG_AllocMem(buf_size);
