...
static void lv_ft_font_destroy_cache(EG_Font_t *font);
#else
...
static void lv_ft_font_destroy_nocache(EG_Font_t *font);
#endif
...
bool lv_freetype_init(uint16_t max_faces, uint16_t max_sizes, uint32_t max_bytes)
{
	...
	if(error) {
		EG_LOG_ERROR("init freeType error(%d)", error);
		return false;
	}
	...
	if(error) {
		...
		EG_LOG_ERROR("Failed to open cache manager");
		return false;
	}

	error = FTC_CMapCache_New(cache_manager, &cmap_cache);
	if(error) {
		EG_LOG_ERROR("Failed to open Cmap Cache");
		goto Fail;
	}
	...
	if(error) {
		EG_LOG_ERROR("Failed to open sbit cache");
		goto Fail;
	}
#else
	error = FTC_ImageCache_New(cache_manager, &image_cache);
	if(error) {
		EG_LOG_ERROR("Failed to open image cache");
		goto Fail;
	}
#endif
																		...
																		FT_Library library_is, FT_Pointer req_data, FT_Face *aface)
{
	...
	if(error) {
		EG_LOG_ERROR("FT_New_Face error:%d\n", error);
		return error;
	}
	return FT_Err_Ok;
}

static bool get_bold_glyph(const EG_Font_t *font, FT_Face face,
													 FT_UInt glyph_index, EG_FontGlyphProps_t *dsc_out)
{
	...
	dsc_out->AdvWidth = (face->glyph->metrics.horiAdvance >> 6);
	dsc_out->BoxHeight = face->glyph->bitmap.rows;  /*Height of the bitmap in [px]*/
	dsc_out->BoxWidth = face->glyph->bitmap.width; /*Width of the bitmap in [px]*/
	dsc_out->OffsetX = face->glyph->bitmap_left;  /*X offset of the bitmap in [pf]*/
	dsc_out->OffsetY = face->glyph->bitmap_top -
		face->glyph->bitmap.rows; /*Y offset of the bitmap measured from the as line*/
	dsc_out->BitsPerPixel = 8;           /*Bit per pixel: 1/2/4/8*/

	return true;
}

static bool get_glyph_dsc_cb_cache(const EG_Font_t *font,
																	 EG_FontGlyphProps_t *dsc_out, uint32_t unicode_letter, uint32_t unicode_letter_next)
{
	EG_UNUSED(unicode_letter_next);
	...
	dsc_out->IsPlaceholder = glyph_index == 0;
	...
	if(error) {
		EG_LOG_ERROR("SBitCache_Lookup error");
		return false;
	}

	dsc_out->AdvWidth = sbit->xadvance;
	dsc_out->BoxHeight = sbit->height;             /*Height of the bitmap in [px]*/
	dsc_out->BoxWidth = sbit->width;              /*Width of the bitmap in [px]*/
	dsc_out->OffsetX = sbit->left;               /*X offset of the bitmap in [pf]*/
	dsc_out->OffsetY = sbit->top - sbit->height; /*Y offset of the bitmap measured from the as line*/
	dsc_out->BitsPerPixel = 8;                          /*Bit per pixel: 1/2/4/8*/
#else
	FT_Error error = FTC_ImageCache_Lookup(image_cache, &desc_type, glyph_index, &image_glyph, NULL);
	if(error) {
		EG_LOG_ERROR("ImageCache_Lookup error");
		return false;
	}
	if(image_glyph->format != FT_GLYPH_FORMAT_BITMAP) {
		EG_LOG_ERROR("Glyph_To_Bitmap error");
		return false;
	}

	FT_BitmapGlyph GlyphBitmap = (FT_BitmapGlyph)image_glyph;
	dsc_out->AdvWidth = (GlyphBitmap->root.advance.x >> 16);
	dsc_out->BoxHeight = GlyphBitmap->bitmap.rows;  /*Height of the bitmap in [px]*/
	dsc_out->BoxWidth = GlyphBitmap->bitmap.width; /*Width of the bitmap in [px]*/
	dsc_out->OffsetX = GlyphBitmap->left;         /*X offset of the bitmap in [pf]*/
	dsc_out->OffsetY = GlyphBitmap->top -
		GlyphBitmap->bitmap.rows; /*Y offset of the bitmap measured from the as line*/
	dsc_out->BitsPerPixel = 8;            /*Bit per pixel: 1/2/4/8*/
#endif

end:
	if((dsc->style & FT_FONT_STYLE_ITALIC) && (unicode_letter_next == '\0')) {
		dsc_out->AdvWidth = dsc_out->BoxWidth + dsc_out->OffsetX;
	}

	return true;
}

static const uint8_t *get_glyph_bitmap_cb_cache(const EG_Font_t *font, uint32_t unicode_letter)
{
	EG_UNUSED(unicode_letter);
...
#else
	FT_BitmapGlyph GlyphBitmap = (FT_BitmapGlyph)image_glyph;
	return (const uint8_t *)GlyphBitmap->bitmap.buffer;
#endif
}

static bool lv_ft_font_init_cache(lv_ft_info_t *info)
{
	size_t need_size = sizeof(lv_font_fmt_ft_dsc_t) + sizeof(EG_Font_t);
	lv_font_fmt_ft_dsc_t *dsc = EG_AllocMem(need_size);
	if(dsc == NULL) return false;
	EG_ZeroMem(dsc, need_size);

	dsc->font = (EG_Font_t *)(((char *)dsc) + sizeof(lv_font_fmt_ft_dsc_t));
	dsc->mem = info->mem;
	...
	if(error) {
		EG_LOG_ERROR("Failed to LookupSize");
		goto Fail;
	}

	EG_Font_t *font = dsc->font;
	font->dsc = dsc;
	font->GetGlyphPropsCB = get_glyph_dsc_cb_cache;
	font->GetGlyphBitmapCB = get_glyph_bitmap_cb_cache;
	font->subpx = EG_FONT_SUBPX_NONE;
	font->LineHeight = (face_size->face->size->metrics.height >> 6);
	font->BaseLine = -(face_size->face->size->metrics.descender >> 6);

	FT_Fixed scale = face_size->face->size->metrics.y_scale;
	int8_t thickness = FT_MulFix(scale, face_size->face->UnderlineThickness) >> 6;
	font->UnderlinePosition = FT_MulFix(scale, face_size->face->UnderlinePosition) >> 6;
	font->UnderlineThickness = thickness < 1 ? 1 : thickness;
...
void lv_ft_font_destroy_cache(EG_Font_t *font)
{
	if(font == NULL) {
		return;
	}
...
static void face_generic_finalizer(void *object)
{
	...
	EG_LOG_INFO("face finalizer(%p)\n", face);
}

static bool get_glyph_dsc_cb_nocache(const EG_Font_t *font,
																		 EG_FontGlyphProps_t *dsc_out, uint32_t unicode_letter, uint32_t unicode_letter_next)
{
	EG_UNUSED(unicode_letter_next);
	if(unicode_letter < 0x20) {
...
static const uint8_t *get_glyph_bitmap_cb_nocache(const EG_Font_t *font, uint32_t unicode_letter)
{
	EG_UNUSED(unicode_letter);
	lv_font_fmt_ft_dsc_t *dsc = (lv_font_fmt_ft_dsc_t *)(font->dsc);
	FT_Face face = dsc->size->face;
...
static bool lv_ft_font_init_nocache(lv_ft_info_t *info)
{
	...
	if(face == NULL) {
		...
		if(error) {
			EG_LOG_WARN("create face error(%d)", error);
			goto Fail;
		}
	...
	EG_Font_t *font = dsc->font;
	font->dsc = dsc;
	font->GetGlyphPropsCB = get_glyph_dsc_cb_nocache;
	font->GetGlyphBitmapCB = get_glyph_bitmap_cb_nocache;
	font->LineHeight = (face->size->metrics.height >> 6);
	font->BaseLine = -(face->size->metrics.descender >> 6);
	font->subpx = EG_FONT_SUBPX_NONE;

	FT_Fixed scale = face->size->metrics.y_scale;
	int8_t thickness = FT_MulFix(scale, face->UnderlineThickness) >> 6;
	font->UnderlinePosition = FT_MulFix(scale, face->UnderlinePosition) >> 6;
	font->UnderlineThickness = thickness < 1 ? 1 : thickness;
...
static void name_refer_del(const char *name)
{
	...
	EG_LOG_WARN("name_in_names_del error(not find:%p).", name);
}
...
static const char *name_refer_save(const char *name)
{
	...
	if(refer) {
		...
		refer->name = EG_AllocMem(len);
		if(refer->name) {
			EG_CopyMem((void *)refer->name, name, len);
			refer->cnt = 1;
			return refer->name;
		}
		_lv_ll_remove(&names_ll, refer);
	...
	EG_LOG_WARN("save_name_to_names error(not memory).");
	return "";
}
